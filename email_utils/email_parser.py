...
def parse_email_content(email_data):
    """
    Parses email data from the Gmail API and extracts sender, subject, body,
    and the message ID.
    """
    try:
        ...
        # Attempt to decode the message body from the payload.
        body = ""
        ...
        # Then try HTML parts if no plain text found.
        if not body and "parts" in payload:
            for part in payload["parts"]:
        ...
        return {"sender": sender, "subject": subject, "body": body, "message_id": message_id}
    except Exception as e:
